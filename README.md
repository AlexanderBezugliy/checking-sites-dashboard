...
Блок **Google Index** (`IndexStrip`) показывает KPI по главным и слотам из `sites.csv` (`bonus`, `app`, …), фильтры таблицы и раскрытие строки с `index.pages[]`. Кнопка «Скачать не в индексе» внизу блока отдаёт CSV (разделитель `;`, для русской Excel) всех страниц с `indexed: false` (слоты из CSV, без skip и без ошибок Google). Дроп (нет в CSV, **301** на money-сайт из того же `status.json`) в колонке индекса — `drop`, не skip и не падение; чужой 301 на хост вне флота остаётся аварией. В шапке отдельно: «Последняя проверка» (аптайм) и «Индекс обновлён» (`index_last_update`). `index.sitemap` — справочник, не список для UI.
